...
class Camera1:
	...
	def img_process(self, img):
		...
		resized_image = cv2.resize(img, (390, 640)) 
	...
	def id_and_priority_calc(self, qrcode, qr_result):
		...
		sku = sku_color+shelf_id+"0121"

		self.list_maker(qrcode, shelf_id, priority, qr_result, sku, storage_no, cost, item)

	def list_maker(self, qrcode, shelf_id, priority, qr_result, sku, storage_no, cost, item):
		global pkg_list
		pkg_id = "packagen"+shelf_id
		camera_name = "logical_camera_2_"
		suffix = "_frame"

		target_frame = camera_name+pkg_id+suffix
