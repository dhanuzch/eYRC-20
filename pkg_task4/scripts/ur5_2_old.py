...
class manipulation:
    ...
    def conveyor_msg(self, msg):
        ...
        self.conv_status = msg.conv_status
        #pkg_id = msg.pkg_id
        #conv_status = msg.conv_status

        self.pkg_name()
        print "*****************************************"
        print "first color thing:", self.pkg_color
        print "*****************************************"

    def pkg_name(self):
        pkg_details = rospy.get_param("/pkg_details")
        pkg_dict = next((item for item in pkg_details if item["pkg_id"] == self.pkg_id), None)
        if pkg_dict == None:
            pass
        else:
    ...
    def ee_move(self):
        ...
        self.target_frame = camera_name+self.pkg_id+suffix
        print "target_frame:", self.target_frame
        ...
        if target_frame != "logical_camera_2__frame" and target_frame != "logical_camera_2_ur5_frame" and target_frame != "logical_camera_2_ur5_2_frame":
            if flag_plan == True:
        ...
        if target_frame != "logical_camera_2__frame" and target_frame != "logical_camera_2_ur5_frame" and target_frame != "logical_camera_2_ur5_2_frame" and len(self.pkg_id) != 0:
            ur5.go_to_pose(pkg_pose)
            self.attach_box()
            if len(self._object_model_name) >=1:
                self.to_bin(self.pkg_color)
            ...
            if len(self._object_model_name) == 0:
                print "nothing lol"
        """
        if self.target_frame != "logical_camera_2__frame" and self.target_frame != "logical_camera_2_ur5_frame" and self.target_frame != "logical_camera_2_ur5_2_frame" and len(self.pkg_id) != 0:
            ...
            if arm_to_box_Status == True:
                ...
                if len(self._object_model_name) >=1:
                    self.attach_box()
                    rospy.sleep(1)
                    arm_to_bin_Status = self.to_bin(self.pkg_color)
    ...
    def ee_home(self):
        home_joint_angles = [math.radians(-3.004),
                    math.radians(0.155),
                    math.radians(-0.938),
                    math.radians(-0.786),
                    math.radians(-1.57),
        ...
        # wait for camera signal...if it's true then break
    def attach_box(self):
        box_model_name = self.pkg_id
    ...
    def joint_angle_set(self, joint1):
        ...
        #return to_bin_joint_set_status
def main():
